...
const CancelOrders = () => {
  ...
  const getOrders = () => {
    try {
      setDataLoader(true);
      axios
        .get(`${baseURL}/order_cancellation_requests`, {
          headers: {
  ...
  const OrderCard = ({ order }) => {
    return (
      ...
      >
        ...
        >
          {`Mobile no. ${order?.creator?.phone_number}`}
        </Text>
        {/* <View
          activeOpacity={0.9}
          //   onPress={onPress}
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.card }}>
      <Header titleLeft leftIcon={"back"} title={"Cancel Orders"} />
      <DeleteModal
